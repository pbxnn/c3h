...
func wireApp(bootstrap *conf.Bootstrap, confData *conf.Data, logger log.Logger) (*C3h, func(), error) {
	...
	dataInfoRepo := data.NewDataInfoRepo(db, logger)
	r401SRepo := data.NewR401SRepo(db, moduleRelationRepo, dataInfoRepo, logger)
	r401SUsecase := biz.NewR401SUsecase(r401SRepo, logger)
	...
	collectRepo := data.NewCollectorRepo(moduleRelationRepo, db, cache, logger)
	collectJob := job.NewCollectJob(bootstrap, logger, collectRepo, auditLogUsecase)
